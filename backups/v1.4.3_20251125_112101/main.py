    ...
    try:
        ...
        for pair in pairs:
            try:
                ...
                setups = detect_all_trading_setups(pair, market_data)  # v1.4.3: Multi-TF scan
...
• Losing Trades: {stats['losing_trades']} ({100 - stats['win_rate']:.1f}% if stats['win_rate'] < 100 else 0}%)
...
def detect_trading_setup(pair, market_data):
    ...
    try:
        ...
        timeframe = '5m' if (volume_5m['spike'] or len(obs_5m) > 0) else '3m'  # v1.4.3
...
"""
        ...
        # FVG Setup Check (benzer mantık, kısaltıyorum)
        if len(fvgs) > 0:
            fvg = fvgs[0]
