...
v2.3.5:
- REMOVED: Fallback volume/ATR hesaplama tamamen kaldırıldı
- CHANGED: Cache'de veri yoksa spike_ratio=0, atr=0 döner
- SIMPLIFIED: Sadece TradingView Binance verisi kullanılıyor
- Setup'lar "Low Volume" ile reject edilecek (cache miss durumunda)

v2.3.4:
- ATR de TradingView cache'den okunuyor
- volume_cache → market_cache (hem Volume hem ATR)

v2.3.3:
- TradingView'dan gelen volume spike cache'den alınıyor

v2.3.0:
- execute_trade_for_setup() - Unified trade execution
"""
...
# v2.3.8: DRY - Volume thresholds calculators'tan
...
# TF bazlı geriye bakılacak bar sayısı (OB/FVG tarama için)
TF_LOOKBACK_BARS = {
    '5m': 288,   # 24 saat
    '15m': 96,   # 24 saat
    '30m': 48,   # 24 saat
...
# v2.3.8: MARKET_CACHE_TTL kaldırıldı → Config.MARKET_CACHE_TTL kullanılıyor
...
class BitgetService:
    """Bitget Borsa Servisi - v2.3.8 (TradingView Only + DRY)"""
    ...
    def __init__(self):
        ...
        try:
            ...
            self._market_cache = {}
            logger.info(f"Bitget Servisi v2.3.8 Hazır | Lev:{Config.DEFAULT_LEVERAGE}x | "
                       f"Cache TTL:{Config.MARKET_CACHE_TTL}s | TradingView Only")
    ...
    def calculate_atr(self, ticker, timeframe, period=14):
        """
        v2.3.5: ATR hesaplama (sadece OB/FVG tarama için kullanılıyor)
        Volume/ATR cache'den gelecek, bu fonksiyon backup
        """
        try:
    ...
    def get_complete_analysis_data(self, ticker, market_cache=None):
        """
        v2.3.8: Market verisi toplama - SADECE TradingView Cache (No Fallback) + DRY
        
        Args:
            ticker: Coin pair (BTCUSDT, BTC/USDT:USDT vb.)
            market_cache: TradingView'dan gelen market cache dict
                         Format: {"BTCUSDT_15m": {"spike": 2.34, "atr": 125.5, "ts": 1702...}, ...}
        
        v2.3.8 DEĞİŞİKLİK:
        - MARKET_CACHE_TTL → Config.MARKET_CACHE_TTL (DRY)
        - Volume spike/strength thresholds → calculators.py'den (DRY)
        """
        try:
            symbol = self._normalize_symbol(ticker)
            coin_name = ticker.replace('/USDT:USDT', '').replace('/USDT', '').replace('USDT', '')
            
            # v2.3.5: Cache için pair format: "BTCUSDT"
            ...
            for tf in Config.TIMEFRAMES:
                if tf in self.SKIP_TIMEFRAMES:
                    continue
                try:
                    # Key'leri API formatına çevir ('5D' → '5m')
                    ...
                    # ============================================
                    # v2.3.8: MARKET CACHE - Config.MARKET_CACHE_TTL + DRY Thresholds
                    # ============================================
                    ...
                    if cache_fresh and cached.get('atr', 0) > 0:
                        ...
                        # v2.3.5: Cache yoksa ATR = 0 (OB/FVG hesaplaması için Bitget'ten çek)
                        # ATR kritik olduğu için bunu tutuyoruz
                        atr_data[api_tf] = self.calculate_atr(symbol, tf, 14)
                        cache_misses += 1
                    
                    # --- VOLUME (v2.3.8: DRY Thresholds!) ---
                    if cache_fresh:
                        # Volume cache'den
                        spike_ratio = cached.get('spike', 0)
                        # v2.3.8: DRY - Threshold'lar calculators.py'den
                        ...
                        volume_data[api_tf] = {
                            'spike': spike,
                            'spike_ratio': round(spike_ratio, 4),  # v2.3.8: .4f precision
                            'strength': strength,
                            'trend': 'unknown',
                            'source': 'tradingview_binance',
                            'cache_age': cache_age
                        }
                        logger.debug(f"📈 {coin_name} [{api_tf}]: TV Cache HIT = {spike_ratio:.4f}x")
                    else:
                        # v2.3.5: NO FALLBACK - Cache yoksa spike_ratio = 0
                        volume_data[api_tf] = {
                            'spike': False,
                            'spike_ratio': 0.0,
                            'strength': 'low',
                        ...
                        logger.debug(f"📉 {coin_name} [{api_tf}]: NO CACHE - spike=0")
            ...
            # v2.3.8: Cache log
            logger.info(f"✅ {coin_name}: Price={format_price_display(current_price)}, OBs={total_obs}, FVGs={total_fvgs} | Cache: {cache_hits} hits, {cache_misses} misses")
    ...
    # ============================================
    # ORDER METHODS - v2.2.9 (SL Clamp Fix)
    # ============================================
    ...
    def calculate_position_size(self, entry_price, stop_price):
        ...
        try:
            ...
            # TOTAL balance kullan (available değil!)
            ...
            logger.info(f"💰 Pozisyon Hesabı (v2.3.8):")
    ...
    def place_order_with_tp_sl(self, symbol, side, entry_price, stop_price, tp_price=None):
        """
        v2.2.9: Bitget Order with TP/SL at Place Time + SL CLAMP FIX
        """
        self._require_auth()
        try:
            symbol = self._normalize_symbol(symbol)
            ...
            # STOP LOSS MİNİMUM MESAFE KONTROLÜ
            ...
            if stop_distance_pct < min_stop_pct:
                if side.lower() == 'sell':
                    ...
                    stop_price = entry_price * (1 - min_stop_pct)
                logger.info(f"📏 SL düzeltildi: {format_price_display(original_stop)} → {format_price_display(stop_price)} (min {min_stop_pct:.1%})")
    ...
    def execute_trade_for_setup(self, setup_data, claude_decision=None):
        ...
        try:
            ...
            if claude_decision:
                logger.info(f"🧠 Claude: {claude_decision.get('action', 'N/A')} ({claude_decision.get('confidence', 0)}%) - {claude_decision.get('reasoning', '')[:50]}")
            ...
            if result.get('success'):
                logger.info(f"✅ Trade executed: {coin_name} {direction} | Order: {result.get('order_id', 'N/A')[:16]}...")
                ...
                logger.error(f"❌ Trade failed: {coin_name} {direction} - {error}")
