...
def index():
    """Root endpoint"""
    return jsonify({
        "service": "T-TARS Trading Bot v1.4",
        "version": "1.4.0",
        "status": "running",
        ...
        for pair in pairs:
            try:
                ...
                if setup_detected:
                    ...
                    take_profit = setup_detected.get('take_profit', 'N/A')
...
def test_telegram():
    ...
    try:
        telegram.send(f"🧪 *T-TARS v1.4 Test*\n\n✅ Sistem çalışıyor!\n\n⏰ {get_turkey_time().strftime('%H:%M:%S')}")
        return jsonify({"status": "success"})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    ...
    try:
        ...
        if has_order_block and has_volume_spike:
            ...
            if bias == 'bullish' and ob['type'] == 'bullish':
                ...
                stop_loss = f"${ob['low'] - stop_distance:,.2f}"
                tp = current_price + (atr_15m * 2.5)
                take_profit = f"${tp:,.2f}"
                
                risk = abs(current_price - (ob['low'] - stop_distance))
...
📈 **TP/Stop Analizi:**
• ATR({timeframe}): ${atr_5m if timeframe == '5m' else atr_3m:,.2f}
• Stop: Below OB - 1.5 ATR
...
"""
                ...
                stop_distance = atr_5m * 1.5 if timeframe == '5m' else atr_3m * 1.5
                stop_loss = f"${ob['high'] + stop_distance:,.2f}"
                tp = current_price - (atr_15m * 2.5)
                take_profit = f"${tp:,.2f}"
                
                risk = abs((ob['high'] + stop_distance) - current_price)
...
if __name__ == '__main__':
    logger.info(f"🚀 Starting T-TARS Trading Bot v1.4.0 on port {Config.PORT}")
